...
<%	
	...
	if(login != null){  
	  	cookieName = boardType + "-" + bidx + "-" +  login.getMidx();   // 게시판번호(lidx) - 게시글번호 - 회원번호(midx)로 쿠키이름만 생성
	}
...
<body>
	<div id="wrap">
		...
		<section>
			...
			<div class="viewJone">
				<h2><%=subject %></h2>
				<small><%=writer %></small>&nbsp;&nbsp;|&nbsp;&nbsp;<small><%=bdate %></small>&nbsp;&nbsp;<%if(login != null){ %><a href="freeboardup.jsp?bidx=<%=bidx%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>&midx=<%=login.getMidx()%>"><i class="bi bi-hand-thumbs-up-fill" style="font-size: 2rem; color: cornflowerblue;"></i></a><%} %>
				...
				<%if(login != null && login.getMidx() == midx) {%> 
					<button type="button" class="btn btn-secondary" onclick="location.href='freeboardmodify.jsp?bidx=<%=bidx%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>'">수정</button>
					<button onclick="deleteFn()" type="button" class="btn btn-secondary">삭제</button>
				<%} %>
					<button type="button" class="btn btn-secondary" onclick="location.href='freeboardlist.jsp?searchType=<%=searchType%>&searchValue=<%=searchValue%>'">목록</button>
				</div>
				...
				<form name="frm" action="freeboarddelete.jsp" method="post">
