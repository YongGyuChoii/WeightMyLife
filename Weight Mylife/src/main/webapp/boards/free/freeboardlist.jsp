	...
	try{
		...
		if(searchValue != null && !searchValue.equals("") && !searchValue.equals("null")){
			if(searchType.equals("fsubcon")){
				...
				sql += " where writer like '%"+searchValue+"%' and boardtype='free' order by bidx desc) b)";
			}else {
				sql += " where boardtype='free' order by bidx desc) b)";
			}
		}else if(viewType != null){
			if(viewType.equals("flikey")){
				sql += " where boardtype='free' order by likey desc) b)";
			}else if(viewType.equals("fview")){
				sql += " where boardtype='free' order by viewcnt desc) b)";
			}else{
				sql += " where boardtype='free' order by bidx desc) b)";
			}
		}else {
...
<script>
	function insertBoard(){
		...
		if(login == 'null'){
			...
			location.href = "freeboardinsert.jsp?searchType=<%=searchType%>&searchValue=<%=searchValue%>";
		}
	}
</script>
<style>
header li:nth-child(1) a:nth-child(1) {
	color: orange;
}
</style>
</head>
		...
		<section>
			...
			<table class="table">
				...
				<tbody>
					...
					<tr class="table-secondary">
						...
						<td><a href="freeboardview.jsp?bidx=<%=rs.getInt("bidx")%>&boardType=<%=rs.getString("boardType") %>&searchType=<%=searchType %>&searchValue=<%=searchValue %>"><%=rs.getString("subject") %></a></td>
			...
			<div class="pagingArea">
				<nav aria-label="Page navigation example">
					<ul class="pagination">
						<li class="page-item"><a class="page-link" <%if(paging.getStartPage() > 1){ %> href="freeboardlist.jsp?nowPage=<%=paging.getStartPage()-1 %>&searchType=<%=searchType %>&searchValue=<%=searchValue %>&viewType=<%=viewType %>" <%} %>
						...
						<%for(int i = paging.getStartPage(); i <= paging.getEndPage(); i++) {%>
							...
							<%}else { %>
								<li class="page-item"><a class="page-link" href="freeboardlist.jsp?nowPage=<%=i%>&searchType=<%=searchType %>&searchValue=<%=searchValue %>&viewType=<%=viewType %>"><%=i %></a></li>
							<%} %>
						<%} %>
						
						<li class="page-item"><a class="page-link" <%if(paging.getEndPage() != paging.getLastPage()) {%> href="freeboardlist.jsp?nowPage=<%=paging.getEndPage()+1 %>&searchType=<%=searchType %>&searchValue=<%=searchValue %>&viewType=<%=viewType %>" <%} %>
