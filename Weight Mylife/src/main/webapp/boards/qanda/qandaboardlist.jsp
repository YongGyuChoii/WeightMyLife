	...
	try{
		...
		if(searchValue != null && !searchValue.equals("") && !searchValue.equals("null")){
			if(searchType.equals("fsubcon")){
				sql += " where (subject like '%"+searchValue+"%' or content like '%"+searchValue+"%') and boardtype='qanda'";
			}else if(searchType.equals("fwriter")){
				sql += " where writer like '%"+searchValue+"%' and boardtype='qanda'";
			}
		}else if(viewType != null){
			if(viewType.equals("flikey")){
				sql += " where boardtype='qanda' order by likey desc";
			}else if(viewType.equals("fview")){
				sql += " where boardtype='qanda' order by viewcnt desc";
			}else{
				sql += " where boardtype='qanda' order by bidx desc";
			}
		}else {
			sql += " where boardtype='qanda' order by bidx desc";
		}
		...
		if(searchValue != null && !searchValue.equals("") && !searchValue.equals("null")){
			if(searchType.equals("fsubcon")){
				sql += " where (subject like '%"+searchValue+"%' or content like '%"+searchValue+"%') and boardtype='qanda' order by bidx desc) b)";
			}else if(searchType.equals("fwriter")){
				sql += " where writer like '%"+searchValue+"%' and boardtype='qanda' order by bidx desc) b)";
			}else {
				sql += " where boardtype='qanda' order by bidx desc) b)";
			}
		}else if(viewType != null){
			if(viewType.equals("flikey")){
				sql += " where boardtype='qanda' order by likey desc) b)";
			}else if(viewType.equals("fview")){
				sql += " where boardtype='qanda' order by viewcnt desc) b)";
			}else{
				sql += " where boardtype='qanda' order by bidx desc) b)";
			}
		}else {
...
<script>
	function insertBoard(){
		...
		if(login == 'null'){
			...
			location.href = "qandaboardinsert.jsp?searchType=<%=searchType%>&searchValue=<%=searchValue%>";
		}
	}
</script>
<style>
	...
	<div id="wrap">
		...
		<section>
			...
			<div class="searchArea1">
				<form action="qandaboardlist.jsp">
			...
			<table class="table">
				...
				<tbody>
					...
					<tr class="table-secondary">
						...
						<td><a href="qandaboardview.jsp?bidx=<%=rs.getInt("bidx")%>&boardType=<%=rs.getString("boardType") %>&searchType=<%=searchType %>&searchValue=<%=searchValue %>"><%=rs.getString("subject") %></a></td>
			...
			<div class="pagingArea">
				<nav aria-label="Page navigation example">
					<ul class="pagination">
						<li class="page-item"><a class="page-link" <%if(paging.getStartPage() > 1){ %> href="qandaboardlist.jsp?nowPage=<%=paging.getStartPage()-1 %>&searchType=<%=searchType %>&searchValue=<%=searchValue %>&viewType=<%=viewType %>" <%} %>
						...
						<%for(int i = paging.getStartPage(); i <= paging.getEndPage(); i++) {%>
							...
							<%}else { %>
								<li class="page-item"><a class="page-link" href="qandaboardlist.jsp?nowPage=<%=i%>&searchType=<%=searchType %>&searchValue=<%=searchValue %>&viewType=<%=viewType %>"><%=i %></a></li>
							<%} %>
						<%} %>
						
						<li class="page-item"><a class="page-link" <%if(paging.getEndPage() != paging.getLastPage()) {%> href="qandaboardlist.jsp?nowPage=<%=paging.getEndPage()+1 %>&searchType=<%=searchType %>&searchValue=<%=searchValue %>&viewType=<%=viewType %>" <%} %>
